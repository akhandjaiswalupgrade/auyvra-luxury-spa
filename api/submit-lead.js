...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 3. Send email to mevryxind@gmail.com and auyvraluxuryspa@gmail.com
    ...
    if (!smtpUser || !smtpPass) {
      console.warn("SMTP credentials (SMTP_USER/SMTP_PASS) not configured. Skipping email send.");
      return res.status(200).json({
        success: true,
        message: "Lead successfully captured! (Email skipped: SMTP env variables missing on server)"
      });
    }
...
This lead has been logged to Vercel console logs.
      `,
      html: `
<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; padding: 25px; border-radius: 12px; background-color: #fafafa;">
  ...
  <div style="margin-top: 30px; text-align: center;">
    ...
    return res.status(200).json({ success: true, message: "Lead saved and emails sent successfully!" });
